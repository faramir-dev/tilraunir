...
use crate::dissector::configuration::{with_configuration, Config};
...
impl Conversation {
    ...
    fn process_unvisited_packet(
        ...
        if counter <= 2 {
            ...
            if self.connection_msgs.len() == 2 {
                with_configuration(|conf| -> Result<(), Error> {
                    match conf {
                        Ok(conf) => {
                            self.upgrade(conf)?;
                            msg(format!("Upgraded peer! {}", self));
                            Ok(())}
                        Err(e) => Err(PeerNotUpgradedError)?
                    }
                });
            }
    ...
    pub fn with<F, R>(key: *const tcp_analysis, f: F) -> R
    where
        F: FnOnce(&mut Self) -> R,
    {
        CONVERSATIONS_MAP.with(|m| -> R {
            let mut conversations_map = m.borrow_mut();
    ...
    pub fn remove<'a>(key: *const tcp_analysis) {
        CONVERSATIONS_MAP.with(|map| map.borrow_mut().remove(&key));
    }
}
...
thread_local!(
    static CONVERSATIONS_MAP: RefCell<HashMap<*const tcp_analysis, Conversation>> = RefCell::new(HashMap::new())
);
